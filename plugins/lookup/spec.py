...
short_description: >-
  This plugin reads the content of given yang document and generates json and
  xml configuration skeleton and a tree structure of yang document.
description:
  - >-
    This plugin parses yang document and generates json and xml configuration
    skeleton and a tree structure of yang document. The tree structure document
    is as per RFC 8340 which helps to consume the yang document along with json
    and xml configuration skeleton.
options:
  _terms:
    description: >-
      The path points to the location of the top level yang module which is to
      be transformed into to Ansible spec.
    required: true
    type: str
  search_path:
    description:
      - >-
        is a colon C(:) separated list of directories to search for imported
        yang modules in the yang file mentioned in C(path) option. If the value
        is not given it will search in the same directory as that of
        C(yang_file).
    type: path
  defaults:
    description:
      - >-
        This boolean flag indicates if the generated json and xml configuration
        schema should have fields initialized with default values or not.
    default: false
  doctype:
    description:
      - >-
        Identifies the root node of the configuration skeleton. If value is
        C(config) only configuration data will be present in skeleton, if value
        is C(data) both config and state data fields will be present in output.
    default: config
    choices:
      - config
      - data
  ...
  annotations:
    description:
      - >-
        The boolean flag identifies if the xml skeleton should have comments
        describing the field or not.
    default: false
    type: bool
  keep_tmp_files:
    description:
      - >-
        ...
        while creating spec should be kept or deleted. If the value is C(true)
        the files will not be deleted else by default all the intermediate files
        will be deleted irrespective of whether task run is successful or not.
        The intermediate files are stored in path C(~/.ansible/tmp/yang/spec),
...
_list:
  ...
  contains:
    tree:
      description: The tree representation of yang schema as per RFC 8340
      returned: success
      type: dict
      sample: |
