...
description:
  - >-
    This plugin lookups the input json configuration, validates it against the
    respective yang data model which is also given as input to this plugin and
    coverts it to xml format which can be used as payload within Netconf rpc.
options:
  _terms:
    description:
      - >-
        Input json configuration file path that adheres to a particular yang
        model.
    required: true
    type: path
  doctype:
    description:
      - >-
        Specifies the target root node of the generated xml. The default value
        is C(config)
    default: config
  yang_file:
    description:
      - >-
        Path to yang model file against which the json configuration is
        validated and converted to xml.
    required: true
    type: path
  search_path:
    description:
